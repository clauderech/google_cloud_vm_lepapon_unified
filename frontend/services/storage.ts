...
export function calculateMaxProduciableFor(productId: string, allProducts: Product[]): number {
  ...
  if (!product) return 0;
  if (product.type === 'insumo' || product.type === 'insumo_bebida' || product.type === 'revenda' || product.type === 'sorvete') {
    ...
    async updateProduct(product: Product): Promise<void> {
      if (USE_API) {
        ...
        const payload = {
          ...
          cost: product.cost,
          stock: product.stock,
          package_quantity: product.packageQuantity ?? 1,
...
function mapProductFromDB(p: any): Product {
  return {
    ...
    packageQuantity: Number(p.package_quantity ?? 1),
